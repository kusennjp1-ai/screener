...
### C98 — 2026-07-25 Opus5で経緯を再監査 → 本番バグ1件＋計測バイアス1件を発見・修正
- **依頼**: 「続けて。opus5でこれまでの経緯も見直して」＝直近実装の批判的再検証。
- **① 表示損切りの上限バグ（本番稼働中・実害あり／879aa35）**: C97で入れた`_display_stop`が`min(50DMA, last*0.92)`＝**緩い方**を採用（docstringは「厳しい方」と記載＝実装が意図と逆）。∴50日線から離れた**伸びたリーダーで「損切り -15%〜-30%」を保護ラインとして表示**＝ミネルヴィニの絶対ルール(MAX_LOSS_PCT=0.08)違反。`max()`＋共有定数`MAX_LOSS_PCT`に修正（`compute_risk_plan`は元々「上限を超えるswing lowは無視」と正しく実装済＝既存コードが正・新規コードのみ逸脱）。action/stop_hitは不変。伸びた/締まった両ケースの回帰テスト追加。
- **② 上場時期バイアス（計測系の重大欠陥／2a06ceb）**: `tradable`が**検証開始バーで1回だけ**構築され、以後上場銘柄は全期間不可視。**窓が長いほど不利になる構造的アーティファクト**。`--pit-universe`（日々の適格判定）を追加・既定オフ・オフ時はcaveatを出力。ついでに`--help`のTypeError（未エスケープ`%`）も修正。
- **★再測定結果（run 30135665125・9年同期間）**: 除外されていたのは**708銘柄＝適格の37%**。除去後: **CAGR 4.3%→11.0%**、最大DD -25.8%→**-18.6%**、Sortino 0.46→**0.93**、期待値 0.14R→**0.38R**、勝率34.8%→39.1%、総リターン+45.4%→**+155.2%**（SPY: 14.8% / -33.7% / 1.01 / +243.6%）。年別も2018 -9.7%→**+14.8%**等。∴**「9年でB&Hに大敗」という前サイクルの結論は約6割が計測アーティファクト**。正しい姿は「上げ相場ではCAGRがやや届かない(11.0 vs 14.8)が、下落の深さは約半分・Sortinoほぼ互角」。市場ゲート無し=CAGR4.7%/DD-31.8%＝**ゲートだけで+6.3pp/13pp**（9年でも成立）。
- **③ 前サイクルの説明訂正**: 「敗因は現金保有の機会損失」は誤り（強気年にマイナス＝機会損失では説明不能）。真因の大半は上記②で、残る劣後は「乗り切れない」であって「負ける」ではない。OBJECTIVE.md/scorecard.jsonを訂正値に更新（スマホにも訂正注記を表示）。
- **④ CIスモークの根本原因（07d1c7c）**: 3PR連続で全6件が「画面が描画されない」で失敗。`webServer.port`＝TCP接続で準備完了と誤判定していたため`url`待ちに変更＋CI用に30s。**ただし修正後も同症状が継続＝仮説は反証**（30s適用済み・サーバー起動正常・12/12失敗）。手元はCI=trueでも通過するため環境依存。調査継続（次: 失敗時のerror-context/traceとdev-server stdoutの採取）。
- **教訓（恒久）**: 戦略の優劣を語る前に**計測系を疑う**。今回は結論の6割が計測由来だった。

### C99 — 2026-07-25 6年窓も再測定 → 看板の「市場超え」を撤回（正直さの是正）
- **6年窓 `--pit-universe` 実測（run 30167396207）**: 除外されていた366銘柄を回復（1570→1936）。結果は**逆方向に動いた**: CAGR **15.2%→10.4%**（SPY12.4%＝**市場超えではなくなった**）・最大DD -13.9%→**-15.8%**（SPYは-24.5%）・Sortino 1.28→**0.94**（SPY1.06）・期待値0.53R→**0.38R**・2022年 -0.1%→**-10.5%**。
- **∴スマホの看板「6年で5指標すべて市場超え」は測定バイアスによる過大評価だった → 撤回・訂正表示**。`strategy-scorecard.json`をヘッドラインごと訂正値に差し替え、カードに**黄色の「訂正:」注記**を新設（`scorecard-correction`・2テスト追加、計7）。375px実レンダ確認済み。
- **一貫した正しい姿（両窓）**: CAGR 10.4/11.0% vs SPY 12.4/14.8%＝**やや届かない**。最大DD -15.8/-18.6% vs -24.5/-33.7%＝**指数の6〜7割**。Sortino 0.94/0.93 vs 1.06/1.01＝**ほぼ互角**。窓依存が消え、**素の実力が確定**した。
- **市場ゲートの価値も再確認**: ゲート無しは5年 CAGR1.2%/DD-35.8%（!）・9年 CAGR4.7%/DD-31.8%。**ゲート単独で5年+9.2pp/9年+6.3pp、DDは20pp/13pp改善**＝引き続き最大のレバー。
- **教訓の実証**: C98で「計測系を疑う」と記載した直後に、**自分に有利な数字も同じバイアスで過大だった**ことが判明。バイアスは片側だけに効くとは限らない＝有利な数字ほど疑う。

### C100 — 2026-07-26 品質順スロット配分を採用＋監査由来の表示バグ3件修正
- **採用（両窓確認）**: `--quality-rank`（10枠をRSだけでなく**検出済みVCP優先**で埋める）。5年 CAGR 10.4→**10.7%**・9年 11.0→**11.5%**、最大DDは5年同値/9年 -18.6→**-18.4%**＝**①改善・②悪化なし**で判定基準クリア。期待値も同等以上。⑤勝率のみ9年で-0.4pp（最下位指標のため上位優先で採用）。**過去の棄却(C72系)は上場時期バイアス下の測定＝誤った棄却だった**ことが確定。
- **製品反映**: スマホ「バックテスト準拠候補」の並びを**VCP優先→RS降順**に変更（採用構成と一致・従来はRS単独＝採用前の既定）。
- **監査由来の修正（wf_cc17bc70）**: ①買いカードの自己矛盾（利確目標が10%上限のシグナルstop基準、表示stopは8%上限のrisk_plan基準＝「stop 92.00 なのに 2R 119.00」）→**表示stopと同一アンカーで算出**（2R=116.00に是正・回帰テスト2件追加）。②**Sortinoの式が非標準**（負リターンの平均まわりの標準偏差＝損失の「ばらつき」を測っていた）→教科書どおりの下方偏差（目標0からの二乗平均平方根・全期間で除算）に修正・手計算一致を確認。③右テールのラベルが「上位10%の**勝ち**」だが計算は**全トレード**の上位10%＝過大主張→表記修正。
- **未修正（記録のみ）**: `signals.MAX_STOP_LOSS_PCT=0.10` と `risk.MAX_LOSS_PCT=0.08` の不一致。直すとシグナル出力が変わり凍結metricに影響＝908ハーネス必須のため据え置き。
- **注意**: Sortino式変更により、式修正前後のrunでSortinoを直接比較してはならない（SPYのSortinoも1.01→1.17と変化）。カード用に同一式での再測定を実施中。

### C103 — 2026-08 リターン式の全項目を実測で潰した（複数エージェント＋反証）

- **目的**: 「ミネルヴィニに匹敵する成績」に向けて、`CAGR ≈ トレード数 × 期待値R × リスク%` の各項を1つずつ実測で検証する。全実験を**事前登録**（予測を書いてから実行）・**両窓必須**・**1銘柄チェック必須**・**採用候補は別エージェントが反証**の規律で実施。
- **棄却1: 累進リスク（確認済み上昇で2倍）** — 両窓。5年 CAGR 10.7→13.6 だが最大DD −15.8→−21.9、9年 11.5→13.1（SPY 14.8に依然負け）・最大DD −18.4→−22.0。反証で判明: **共通108トレードで max|ΔR|=0.0000・決済日相違 0/108 ＝ 同一トレード列を2倍で張っただけ**。**SIMO 1銘柄が改善の65.5%**（上位3件で113%＝残り135件は合計マイナス）。除くと 7.5→8.7% で共に大敗。ドローダウンは2022年ベアから**2024/07〜2025/07の上昇相場の中の穴へ移動**。
- **棄却2: 強さへの利確（C67/C68 の再測定）** — 両窓。全部利確 5年12.1/9年**9.1**、半分利確 5年9.4/9年**7.8**。5年窓だけなら全部利確は魅力的（CAGR+1.4pp・Sortino1.33）＝**片窓判定は必ず誤る**。指標の罠も記録: 半分利確は期待値1.72R・勝率61.5%と見えるがCAGRは悪化（分割決済ごとに別トレードが記録されトレード数158→340）。
- **棄却3: 検出率の向上（C102 の base_anchored を売買経路へ）** — 両窓。5年 CAGR 10.7→**8.0**（最大DD −20.9）、9年 11.5→**8.2**（−21.8）。増えたトレード(+12/+46)が損を出す。**908の再現率が測るのは「彼が買った日に反応するか」であって「この執行で利益になるか」ではない**。→ 検出器は表示・並び順では採用、売買経路では無効。**「再現率＝改善の梃子」という前提を明文で否定**（OBJECTIVE.md に追記）。
- **模擬の上限は存在しない**（監査）: 期待値+0.1Rあたり CAGR +2.7/+3.0pp で線形。疑った3件は実測で否定（10枠は0日しか埋まらない／再エントリーは可能／決済ラグ±0.1pp）。約定は+2.24%高いが**回収不能**（ピボット最寄りレーンは両窓とも損失）。**実現リスクは1.25%でなく0.874%**で、正しい式は実測CAGRを±0.5ppで再現＝説明のつかない漏れは無い。**投下資本あたりは年17.7%**（SPYは100%投資で12.4%）。
- **計測系の修正5件**: (1) `--pit-universe` が `--funnel product` 下で無効化＝「点時点に見えて凍結として振る舞う」実行を修正、(2) フラグ未記録を修正＋出荷版と不一致な実行に警告、(3) 建玉サイズの二重定義を統一（スキャナ1.25→2.5% / 書き出し常に1.25%＝**スケール値は誰にも表示されない死んだ分岐**）＋再分岐検出テスト、(4) `risk.py` の誤った「両窓改善・DD不変」を実測とともに撤回、(5) `--cash-yield-pct` 追加時、**対照にも同率を払う**よう修正（片方だけだと選択の寄与が水増しされる: 4.0pp→3.6pp）。
- **OBJECTIVE.md の根拠誤りを訂正**: 「大勝ちを切らない」の根拠が C71/76/80/82/85 とされていたが**5件とも勝ち玉を切っていない**（C76は別ハーネスの検出系、C80はむしろ採用）。実際に切る C67/C68 を両窓で測り直した結果に差し替え。ルールは維持、根拠が正しくなった。
- **次**: リターン式は一巡した。未着手は (a) ユニバース（≥$2Bの1,570銘柄のみ＝**小型の右テールが構造的に不在**・別データ源が必要）、(b) ファンダ条件（point-in-time未取得）、(c) 保有期間/決済ルール。
