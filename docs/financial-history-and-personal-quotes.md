...
The entry panel now supports a personal Finnhub key, entered directly in the user's browser. The key stays in component memory, is cleared from the input after connection, is never in storage/query-cache/public builds, and is sent only to Finnhub's API/WebSocket. REST authentication uses the token header; the official WebSocket protocol requires a token query parameter. Quotes are not published in daily files or redistributed through a shared server.
