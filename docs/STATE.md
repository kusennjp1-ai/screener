...
- **サイクル**: C70採用＋PR #58マージ済み（MA-tight製品統合・FIRE±5 88.6→91.2）／ C71完了（同経路を戦術自動売買に配線→6y−48pp/10y+58ppで**窓間不一致・デフォルト不採用**、フラグ残置）。**核心の区別: recall向上は表示(C70採用)に価値・全自動売買(C71)は枠希釈で窓依存**／ **次候補: 21EMA押し目エントリー(B6)・RS床90(A2)・製品PWAでの新検出源(ma_tight)可視化・高速配信2回目実測**
