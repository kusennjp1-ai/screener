...
- **サイクル**: C31 完了（デイリーダイジェストにポジション要約＋孤児DigestTab実装）／ **次: C32**
...
1. **C32候補a: 静的ビューアのモバイル向けシグナルバッジ**（C30はデスクトップのみカード表示）。
2. **C32候補b: ポジションアラートのpush通知化**（celery beatで日次digest markdown生成→配信チャネル）。
