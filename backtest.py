...
- 市場環境がBUY相当の日のみ新規買い。買いゾーン (ピボット〜+5%) 内のみ
- 1銘柄 = 資金の10% (最大10銘柄)、損切りはスクリーナーと同じストップ (3〜8%)
- +12%で損切りを建値へ引き上げ / +22%で利確 (強さへの売り)
- 60営業日で停滞手仕舞い / 市場環境がDO NOT BUY相当で全ポジション清算
"""
...
def simulate(data, capital=1_000_000.0, slots=SLOTS, target=TARGET,
    ...
    spy_c = spy["Close"].reindex(dates).ffill()
    start = 260
    ...
    def equity(i):
        ...
        for sym, p in positions.items():
            ...
            val += p["shares"] * (float(px) if px == px else p["entry"])
        return val
    ...
    for i in range(start, len(dates)):
        ...
        # --- 週次: 市場環境の更新と新規買い
        if (i - start) % eval_step == 0:
            env = env_state(spy, min(i, len(spy) - 1))
            if env == "DONT" and positions:
                for sym in list(positions):
                    col = m["closes"].columns.get_loc(sym)
                    c = float(m["closes"].iat[i, col])
                    if c == c:
                        close_pos(sym, i, c, "市場環境悪化")
            elif env == "BUY" and len(positions) < slots:
                eq = equity(i)
                for sym, px, stop in candidates_at(m, i):
                    ...
                    shares = alloc / px
                    cash -= shares * px
                    positions[sym] = {"entry": px, "stop": stop, "shares": shares,
    ...
    for sym in list(positions):
        ...
        close_pos(sym, last, c if c == c else positions[sym]["entry"], "末日清算")
...
def summarize(trades, equity_curve, capital, spy_c, dates, start):
    ...
    stats = {
        ...
        "トレード数": len(trades), "勝率%": round(win_rate, 1),
    ...
    return {"stats": stats, "trades": trades, "equity": equity_curve[::5]}
