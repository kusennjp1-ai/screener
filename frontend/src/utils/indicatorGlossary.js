...
export const INDICATOR_GLOSSARY = {
  ...
  fundamental_bonus: {
    title: 'Fnd Bonus（ファンダメンタル・ボーナス）',
    jp: 'Minerviniが重視するファンダ5項目をMinerviniスコアに上限+10点で加点：Code 33（+4）／四半期EPS成長 +40%以上で+2.5・+25%以上で+1.5／売上成長 +25%以上で+1.5・+10%以上で+0.5／ROE 17%以上で+1／EPS Rating 80以上で+1。',
    how: 'テクニカルが最終審判：テンプレート合否は変えず、合格銘柄の並び順だけを押し上げる。データ欠損は中立0（減点なし）。',
  },
...
export const EXECUTION_STATE_GLOSSARY = {
  ...
  unknown: {
    title: 'Unknown（判定不能）',
    jp: '価格データが不足していて実行状態を分類できなかった状態。State Cap（実行状態によるレーティング上限）は適用されません。',
    how: 'データが揃えば自動的に7状態のいずれかに分類されます。',
  },
};
