...
export function entryReadiness(row, date, market, now = Date.now()) {
  ...
  const rules = [
    ...
    check('price','買い位置', finite(row.current_price) && finite(row.se_pivot_price) && row.se_pivot_price > 0 ? row.current_price >= row.se_pivot_price && row.current_price <= row.se_pivot_price * 1.05 : null, `日次価格 ${row.current_price ?? '未確認'} / ピボット ${row.se_pivot_price ?? '未確認'}。0〜5%はこのモデルの設定`),
