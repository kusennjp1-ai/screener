...
for (const row of merged) {
  ...
  rows.set(row.symbol, { ...row, technical_audit: audit });
}
...
const verification = [...rows.values()].map(row => ({ symbol: row.symbol, audit: row.technical_audit,
  methods: Object.fromEntries(['minervini', 'ibd'].map(method => { const a = rankCandidates([row], method)[0]?.assessment; return [method, a || null]; })) }));
...
await writeFile('public/research-daily.json', JSON.stringify({
  schema_version: 1, rule_version: 'research-v3-verified', as_of_date: scan.as_of_date,
